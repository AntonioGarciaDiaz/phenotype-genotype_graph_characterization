    ...
    for opt_al in ["adam", "sgd", "rmsprop"]:
        for n_lay in [2,3,4]:
            for n_neu in [16,32,64]:
                ...
                neural_model.fit(training_input, training_output, epochs=500, batch_size=10, verbose=0)
                # Calculate predictions and round them
                predictions = neural_model.predict(validation_input)
                rnd_predictions = []
                for i in range(len(predictions)):
                    new_prediction = (int(round(predictions[i][0])),validation_output[i])
